...
export function dampColor(out: THREE.Color, target: THREE.Color, lambda: number, dt: number): void {
...
export function isTargetNear(opacity: number, targetOpacity: number): boolean {
  return Math.abs(opacity - targetOpacity) < 0.005;
}
